...
#
#   cd /opt/voice-agent && bash backend/deploy/azure/backend-setup.sh
#
# Prereqs: cp backend/.env.azure backend/.env  (then edit)
set -euo pipefail
...
if [ ! -f backend/.env ]; then
  echo "!! $APP_DIR/backend/.env missing — run: cp backend/.env.azure backend/.env" >&2
  exit 1
fi
...
LLM_MODEL="$(grep -E '^LLM_MODEL=' backend/.env | cut -d= -f2 | tr -d ' ')"
...
cd "$APP_DIR"
sudo cp backend/deploy/azure/voiceai-backend.service /etc/systemd/system/
